...
Engine::SceneObjectHandle Engine::Scene::InstantiateEnviro(std::string ModelPath) {
	Assimp::Importer importer;
	const aiScene* scene = importer.ReadFile(ModelPath, aiProcess_Triangulate | aiProcess_FlipUVs);
	if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) {
		LogR("ERROR ASSIMP -> ", importer.GetErrorString());
	...
	string directory = ModelPath.substr(0, ModelPath.find_last_of('/'));
	SceneObjectHandle handle;
	ProcessModelNodeEnviro(scene->mRootNode, scene, handle, SceneObjectHandle(), directory);
	return handle;
}
