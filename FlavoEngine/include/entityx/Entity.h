...
public:
  ...
  std::bitset<Engine::MAX_COMPONENTS> component_mask() const;
 ...
 */
class EntityManager : Engine::help::NonCopyable {
 public:
  typedef std::bitset<Engine::MAX_COMPONENTS> ComponentMask;
  ...
  class UnpackingView {
   public:
    struct Unpacker {
      ...
      void unpack(Engine::Entity &entity) const {
        unpack_<0, Components...>(entity);
      }

    private:
      template <int N, typename C>
      void unpack_(Engine::Entity &entity) const {
...
template<typename C, typename EM>
inline bool ComponentHandle<C, EM>::IsDefault() const {
	return id_ == 0;
}
